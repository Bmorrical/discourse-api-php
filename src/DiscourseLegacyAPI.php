...
class DiscourseLegacyAPI
{
     ...
     */
    public function __construct($apiHost, $apiKey = null, $protocol = 'http')
    {
     ...
     * approveUser
     *
     * @param integer $userId id of user to approve
     *
    ...
    {
        return $this->_putRequest("/admin/users/{$userId}/approve", array());
    }
    ...
    {
        $paramArray['api_key'] = $this->apiKey;
        $paramArray['api_username'] = $apiUser;
        $paramArray['show_emails'] = 'true';
        $ch = curl_init();
        $url = sprintf('%s://%s%s?%s', $this->protocol, $this->apiHost, $reqString, http_build_query($paramArray));
    ...
    {
        $ch = curl_init();
        $url = sprintf('%s://%s%s?api_key=%s&api_username=%s', $this->protocol, $this->apiHost, $reqString, $this->apiKey, $apiUser);
        curl_setopt($ch, CURLOPT_URL, $url);
        $query = '';
